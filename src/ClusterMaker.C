...
//==========CLICpix calibration option - S.Green
    if (chip == "CLi-CPix")
    {
        std::cout << "==== Cluster Maker ==== CLi-CPix Calibration" << std::endl;
        std::string runNumber =  parameters->eventFile;
        runNumber.erase(0, 3);
        runNumber.erase(runNumber.size() - 5);
        //std::cout << "Run Number : " << runNumber << std::endl;
        TimepixCalibration *pTimepixCalibration = new TimepixCalibration(chip, parameters->pixelcalibration, runNumber);
        calibration[chip] = pTimepixCalibration;
    }
//==========
  }
